...
class ElevenLabsTextToSpeech(TextToSpeech):
    def transform(self, text: str, voice_id: str, save: bool = False, output_folder: str = "./"):
        ...
        output_path = output_folder + "elevenlabs_" + str(timestamp) + ".mp3"
...
voices = {
    "Male-Pleasant": "aQROLel5sQbj1vuIVi6B"
}

voice_id_eleven = voices["Male-Pleasant"]

def get_speech(text, output_folder: str, voice_id = voice_id_eleven):
    t2s = ElevenLabsTextToSpeech()
    path = t2s.transform(text, voice_id, save=True, output_folder=output_folder)
